...
| **3** | Video/Audio settings backend (`UOgmMgaGameUserSettings` + Scalability + submixes) | I write C++, Peter wires UMG | M | **DONE** (P4 113/114), verified in Standalone — Video + UI-scale + 8-channel submix audio + the `WBP_SettingsTabBase` tab framework. **Packaged-build verification of the AUDIO sliders is DEFERRED TO M2 (Peter, 2026-07-27):** with no sound in an M1 build, a submix slider that silently fails and one that works are indistinguishable, so the check is meaningless until M2's ambient bed and footsteps exist — it lands in M2 Pillar C. Polish remains under #16 |
